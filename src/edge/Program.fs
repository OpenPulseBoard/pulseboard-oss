...
let main argv =
  ...
  // Hosted-only modes now live in src/cloud/. Keep this guard near the
  // top so operators get a clear redirect before the workspace runtime
  // starts allocating its normal state.
  ...
  match argv |> Array.tryFind (fun a -> a = "--site-only" || a = "--mode=provisioner") with
  | Some mode ->
    let cloudProj = Path.Combine("src", "cloud", "PulseBoard.Cloud.fsproj")
    eprintfn "  [ERROR] %s is no longer handled by src/edge/Program.fs" mode
    eprintfn "          run the cloud executable instead: dotnet run --project %s -- %s" cloudProj mode
    exit 2
  | None -> ()
  ...
  // Provisioner-owned bearer used for workspace bootstrap key issuance.
  // When unset, `/api/bootstrap/keys` is not mounted.
    ...
    choose [
      ...
      (if multiTenant then
         PulseBoard.Signup.webPart
           tenantStore (PulseBoard.Signup.defaultLimiter ()) auditLog workspaceBootstrapToken
