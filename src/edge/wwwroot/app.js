...
const LIBRARY_CATALOG = [
  ...
  },
  {
    ...
    metrics: [
      ...
      "aws_cloudwatch_request_count",
      "aws_cloudwatch_database_connections"
    ],
    alerts: { name:"AWS Alerts", intervalMs:60000, rules:[
      { name:"Lambda error rate > 5%", lang:"promql",
        expr:'sum by (Region, AccountId) (rate(aws_cloudwatch_errors{Namespace="AWS/Lambda"}[5m])) / sum by (Region, AccountId) (rate(aws_cloudwatch_invocations{Namespace="AWS/Lambda"}[5m])) * 100',
        cmp:">", threshold:5, forMs:300000, severity:"warning",
        labels:{integration:"aws-overview"},
        annotations:{summary:"Lambda error rate > 5% in {{ $labels.Region }} / {{ $labels.AccountId }}"} },
      { name:"API Gateway 5xx sustained", lang:"promql",
        expr:'sum by (Region, AccountId) (aws_cloudwatch_5_xx_error{Namespace="AWS/ApiGateway"})',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"aws-overview"},
        annotations:{summary:"API Gateway returning 5xx in {{ $labels.Region }} / {{ $labels.AccountId }}"} },
      { name:"ALB target 5xx sustained", lang:"promql",
        expr:'sum by (Region, AccountId) (aws_cloudwatch_httpcode_target_5_xx_count{Namespace="AWS/ApplicationELB"})',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"aws-overview"},
        annotations:{summary:"ALB targets returning 5xx in {{ $labels.Region }} / {{ $labels.AccountId }}"} },
      { name:"RDS CPU > 90%", lang:"promql",
        expr:'aws_cloudwatch_cpu_utilization{Namespace="AWS/RDS"}',
        cmp:">", threshold:90, forMs:300000, severity:"critical",
        labels:{integration:"aws-overview"},
        annotations:{summary:"RDS CPU > 90% in {{ $labels.Region }} / {{ $labels.AccountId }}"} },
      { name:"SQS oldest message > 5 min", lang:"promql",
        expr:'aws_cloudwatch_approximate_age_of_oldest_message{Namespace="AWS/SQS"}',
        cmp:">", threshold:300, forMs:300000, severity:"warning",
        labels:{integration:"aws-overview"},
        annotations:{summary:"SQS oldest-message age > 5min in {{ $labels.Region }} / {{ $labels.AccountId }} — queue may be stuck"} },
      { name:"DynamoDB throttled requests", lang:"promql",
        expr:'sum by (Region, AccountId) (aws_cloudwatch_throttled_requests{Namespace="AWS/DynamoDB"})',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"aws-overview"},
        annotations:{summary:"DynamoDB throttling requests in {{ $labels.Region }} / {{ $labels.AccountId }}"} },
    ]},
    dashboard: { title:"AWS Overview", timeRangeSec:3600, refreshSec:60,
      vars:[
        // Populated from labels stamped by the cloud-poller on every
        // CloudWatch sample (cloud-poller/Poller.fs `extraLabels`).
        { name:"region", type:"query", label:"Region",
          query:"label_values(aws_cloudwatch_invocations, Region)",
          options:"", regex:"",
          multi:true, allOption:true, current:["$__all"], hide:false },
        { name:"account", type:"query", label:"AWS account",
          query:"label_values(aws_cloudwatch_invocations, AccountId)",
          options:"", regex:"",
          multi:true, allOption:true, current:["$__all"], hide:false },
      ],
      panels:[
        // -- Row 1: top-line stats (h=2) --
        { id:"p1", title:"AWS accounts (in selection)", type:"stat", queryLang:"promql",
          expr:'count(count by (AccountId) (aws_cloudwatch_invocations{Region=~"$region",AccountId=~"$account"}))',
          x:0,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p2", title:"Regions reporting", type:"stat", queryLang:"promql",
          expr:'count(count by (Region) (aws_cloudwatch_invocations{Region=~"$region",AccountId=~"$account"}))',
          x:3,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p3", title:"Lambda invocations / sec", type:"stat", queryLang:"promql",
          expr:'sum(rate(aws_cloudwatch_invocations{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"}[5m]))',
          x:6,y:0,w:3,h:2, options:{unit:"ops"}, links:[] },
        { id:"p4", title:"ALB requests / sec", type:"stat", queryLang:"promql",
          expr:'sum(rate(aws_cloudwatch_request_count{Namespace="AWS/ApplicationELB",Region=~"$region",AccountId=~"$account"}[5m]))',
          x:9,y:0,w:3,h:2, options:{unit:"ops"}, links:[] },

        // -- Row 2: EC2 --
        { id:"p5", title:"EC2 CPU % (avg, by region)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_cpu_utilization{Namespace="AWS/EC2",Region=~"$region",AccountId=~"$account"})',
          x:0,y:2,w:6,h:3, options:{unit:"%"}, links:[] },
        { id:"p6", title:"EC2 NetworkIn (sum bytes/poll, by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_network_in{Namespace="AWS/EC2",Region=~"$region",AccountId=~"$account"})',
          x:6,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },
        { id:"p7", title:"EC2 NetworkOut (sum bytes/poll, by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_network_out{Namespace="AWS/EC2",Region=~"$region",AccountId=~"$account"})',
          x:9,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 3: Lambda --
        { id:"p8", title:"Lambda invocations (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_invocations{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"})',
          x:0,y:5,w:3,h:3, options:{}, links:[] },
        { id:"p9", title:"Lambda errors (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_errors{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"})',
          x:3,y:5,w:3,h:3, options:{}, links:[] },
        { id:"p10", title:"Lambda error rate %", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (rate(aws_cloudwatch_errors{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"}[5m])) / sum by (Region) (rate(aws_cloudwatch_invocations{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"}[5m])) * 100',
          x:6,y:5,w:3,h:3, options:{unit:"%"}, links:[] },
        { id:"p11", title:"Lambda avg duration (ms)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_duration{Namespace="AWS/Lambda",Region=~"$region",AccountId=~"$account"})',
          x:9,y:5,w:3,h:3, options:{unit:"ms"}, links:[] },

        // -- Row 4: API Gateway --
        { id:"p12", title:"API Gateway requests (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_count{Namespace="AWS/ApiGateway",Region=~"$region",AccountId=~"$account"})',
          x:0,y:8,w:3,h:3, options:{}, links:[] },
        { id:"p13", title:"API Gateway 4xx (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_4_xx_error{Namespace="AWS/ApiGateway",Region=~"$region",AccountId=~"$account"})',
          x:3,y:8,w:3,h:3, options:{}, links:[] },
        { id:"p14", title:"API Gateway 5xx (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_5_xx_error{Namespace="AWS/ApiGateway",Region=~"$region",AccountId=~"$account"})',
          x:6,y:8,w:3,h:3, options:{}, links:[] },
        { id:"p15", title:"API Gateway avg latency (ms)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_latency{Namespace="AWS/ApiGateway",Region=~"$region",AccountId=~"$account"})',
          x:9,y:8,w:3,h:3, options:{unit:"ms"}, links:[] },

        // -- Row 5: ALB --
        { id:"p16", title:"ALB request count (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_request_count{Namespace="AWS/ApplicationELB",Region=~"$region",AccountId=~"$account"})',
          x:0,y:11,w:4,h:3, options:{}, links:[] },
        { id:"p17", title:"ALB target 5xx (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_httpcode_target_5_xx_count{Namespace="AWS/ApplicationELB",Region=~"$region",AccountId=~"$account"})',
          x:4,y:11,w:4,h:3, options:{}, links:[] },
        { id:"p18", title:"ALB target response time (s, avg)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_target_response_time{Namespace="AWS/ApplicationELB",Region=~"$region",AccountId=~"$account"})',
          x:8,y:11,w:4,h:3, options:{unit:"s"}, links:[] },

        // -- Row 6: RDS --
        { id:"p19", title:"RDS CPU % (by region)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_cpu_utilization{Namespace="AWS/RDS",Region=~"$region",AccountId=~"$account"})',
          x:0,y:14,w:4,h:3, options:{unit:"%"}, links:[] },
        { id:"p20", title:"RDS connections (max, by region)", type:"timeseries", queryLang:"promql",
          expr:'max by (Region) (aws_cloudwatch_database_connections{Namespace="AWS/RDS",Region=~"$region",AccountId=~"$account"})',
          x:4,y:14,w:4,h:3, options:{}, links:[] },
        { id:"p21", title:"RDS freeable memory (bytes, by region)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_freeable_memory{Namespace="AWS/RDS",Region=~"$region",AccountId=~"$account"})',
          x:8,y:14,w:4,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 7: SQS --
        { id:"p22", title:"SQS messages visible (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_approximate_number_of_messages_visible{Namespace="AWS/SQS",Region=~"$region",AccountId=~"$account"})',
          x:0,y:17,w:6,h:3, options:{}, links:[] },
        { id:"p23", title:"SQS oldest message age (s, max)", type:"timeseries", queryLang:"promql",
          expr:'max by (Region) (aws_cloudwatch_approximate_age_of_oldest_message{Namespace="AWS/SQS",Region=~"$region",AccountId=~"$account"})',
          x:6,y:17,w:6,h:3, options:{unit:"s"}, links:[] },

        // -- Row 8: DynamoDB --
        { id:"p24", title:"DynamoDB read capacity (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_consumed_read_capacity_units{Namespace="AWS/DynamoDB",Region=~"$region",AccountId=~"$account"})',
          x:0,y:20,w:4,h:3, options:{}, links:[] },
        { id:"p25", title:"DynamoDB write capacity (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_consumed_write_capacity_units{Namespace="AWS/DynamoDB",Region=~"$region",AccountId=~"$account"})',
          x:4,y:20,w:4,h:3, options:{}, links:[] },
        { id:"p26", title:"DynamoDB throttled requests", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_throttled_requests{Namespace="AWS/DynamoDB",Region=~"$region",AccountId=~"$account"})',
          x:8,y:20,w:4,h:3, options:{}, links:[] },

        // -- Row 9: Step Functions --
        { id:"p27", title:"Step Functions started (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_executions_started{Namespace="AWS/States",Region=~"$region",AccountId=~"$account"})',
          x:0,y:23,w:3,h:3, options:{}, links:[] },
        { id:"p28", title:"Step Functions succeeded (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_executions_succeeded{Namespace="AWS/States",Region=~"$region",AccountId=~"$account"})',
          x:3,y:23,w:3,h:3, options:{}, links:[] },
        { id:"p29", title:"Step Functions failed (by region)", type:"timeseries", queryLang:"promql",
          expr:'sum by (Region) (aws_cloudwatch_executions_failed{Namespace="AWS/States",Region=~"$region",AccountId=~"$account"})',
          x:6,y:23,w:3,h:3, options:{}, links:[] },
        { id:"p30", title:"Step Functions avg exec time (ms)", type:"timeseries", queryLang:"promql",
          expr:'avg by (Region) (aws_cloudwatch_execution_time{Namespace="AWS/States",Region=~"$region",AccountId=~"$account"})',
          x:9,y:23,w:3,h:3, options:{unit:"ms"}, links:[] },
      ]}
  },
  {
    id: "azure-overview", icon: "🟦", title: "Azure Overview", category: "Cloud",
    description: "Subscription-wide rollups for VMs, App Service, Storage, SQL DB, AKS, Cosmos DB, Service Bus, Event Hub, Key Vault, and Redis Cache — sourced from Azure Monitor via the pulseboard-cloud poller (60s metrics query against the resources you listed in the `azure/monitor` integration). Filter the whole dashboard by $subscription. Requires: a connected Azure account in Portal → Cloud with the `azure/monitor` integration enabled (Workload Identity Federation, no client secret stored). Series carry labels ResourceType/ResourceName/Aggregation/SubscriptionId/Workspace. The `azure_monitor_*` names are PulseBoard's internal Prom-style series names; Azure metric names are preserved in label values like ResourceType, and the snake-cased Azure metric name forms the series base.",
    metrics: [
      "azure_monitor_percentage_cpu",
      "azure_monitor_requests",
      "azure_monitor_transactions",
      "azure_monitor_cpu_percent"
    ],
    alerts: { name:"Azure Alerts", intervalMs:60000, rules:[
      { name:"VM CPU > 90%", lang:"promql",
        expr:'azure_monitor_percentage_cpu{ResourceType="Microsoft.Compute/virtualMachines"}',
        cmp:">", threshold:90, forMs:300000, severity:"warning",
        labels:{integration:"azure-overview"},
        annotations:{summary:"Azure VM CPU > 90% on {{ $labels.ResourceName }} ({{ $labels.SubscriptionId }})"} },
      { name:"App Service 5xx sustained", lang:"promql",
        expr:'sum by (SubscriptionId, ResourceName) (azure_monitor_http5xx{ResourceType="Microsoft.Web/sites"})',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"azure-overview"},
        annotations:{summary:"App Service {{ $labels.ResourceName }} returning 5xx ({{ $labels.SubscriptionId }})"} },
      { name:"SQL DB CPU > 85%", lang:"promql",
        expr:'azure_monitor_cpu_percent{ResourceType="Microsoft.Sql/servers/databases"}',
        cmp:">", threshold:85, forMs:300000, severity:"critical",
        labels:{integration:"azure-overview"},
        annotations:{summary:"Azure SQL DB CPU > 85% on {{ $labels.ResourceName }} ({{ $labels.SubscriptionId }})"} },
      { name:"AKS node CPU pressure", lang:"promql",
        expr:'azure_monitor_node_cpu_usage_percentage{ResourceType="Microsoft.ContainerService/managedClusters"}',
        cmp:">", threshold:85, forMs:300000, severity:"warning",
        labels:{integration:"azure-overview"},
        annotations:{summary:"AKS node CPU > 85% on cluster {{ $labels.ResourceName }} ({{ $labels.SubscriptionId }})"} },
      { name:"Storage availability degraded", lang:"promql",
        expr:'azure_monitor_availability{ResourceType="Microsoft.Storage/storageAccounts"}',
        cmp:"<", threshold:99.9, forMs:300000, severity:"warning",
        labels:{integration:"azure-overview"},
        annotations:{summary:"Storage account {{ $labels.ResourceName }} availability < 99.9% ({{ $labels.SubscriptionId }})"} },
      { name:"Redis memory > 90%", lang:"promql",
        expr:'azure_monitor_usedmemorypercentage{ResourceType="Microsoft.Cache/Redis"}',
        cmp:">", threshold:90, forMs:300000, severity:"critical",
        labels:{integration:"azure-overview"},
        annotations:{summary:"Azure Redis memory > 90% on {{ $labels.ResourceName }} ({{ $labels.SubscriptionId }})"} },
    ]},
    dashboard: { title:"Azure Overview", timeRangeSec:3600, refreshSec:60,
      vars:[
        // Populated from labels stamped by the cloud-poller on every
        // Azure Monitor sample (cloud-poller/Poller.fs `extraLabels`).
        { name:"subscription", type:"query", label:"Subscription",
          query:"label_values(azure_monitor_percentage_cpu, SubscriptionId)",
          options:"", regex:"",
          multi:true, allOption:true, current:["$__all"], hide:false },
        { name:"resource", type:"query", label:"Resource",
          query:"label_values(azure_monitor_percentage_cpu, ResourceName)",
          options:"", regex:"",
          multi:true, allOption:true, current:["$__all"], hide:false },
      ],
      panels:[
        // -- Row 1: top-line stats --
        { id:"p1", title:"Subscriptions reporting", type:"stat", queryLang:"promql",
          expr:'count(count by (SubscriptionId) (azure_monitor_percentage_cpu{SubscriptionId=~"$subscription"}))',
          x:0,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p2", title:"Resource types reporting", type:"stat", queryLang:"promql",
          expr:'count(count by (ResourceType) ({__name__=~"azure_monitor_.+",SubscriptionId=~"$subscription"}))',
          x:3,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p3", title:"App Service requests / sec (sum)", type:"stat", queryLang:"promql",
          expr:'sum(rate(azure_monitor_requests{ResourceType="Microsoft.Web/sites",SubscriptionId=~"$subscription"}[5m]))',
          x:6,y:0,w:3,h:2, options:{unit:"ops"}, links:[] },
        { id:"p4", title:"Storage transactions / sec (sum)", type:"stat", queryLang:"promql",
          expr:'sum(rate(azure_monitor_transactions{ResourceType="Microsoft.Storage/storageAccounts",SubscriptionId=~"$subscription"}[5m]))',
          x:9,y:0,w:3,h:2, options:{unit:"ops"}, links:[] },

        // -- Row 2: Virtual Machines --
        { id:"p5", title:"VM CPU % (avg, by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_percentage_cpu{ResourceType="Microsoft.Compute/virtualMachines",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:2,w:6,h:3, options:{unit:"%"}, links:[] },
        { id:"p6", title:"VM Network In (bytes/poll, by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_network_in_total{ResourceType="Microsoft.Compute/virtualMachines",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:6,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },
        { id:"p7", title:"VM Network Out (bytes/poll, by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_network_out_total{ResourceType="Microsoft.Compute/virtualMachines",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:9,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 3: App Service --
        { id:"p8", title:"App Service requests (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_requests{ResourceType="Microsoft.Web/sites",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:5,w:3,h:3, options:{}, links:[] },
        { id:"p9", title:"App Service 5xx (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_http5xx{ResourceType="Microsoft.Web/sites",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:3,y:5,w:3,h:3, options:{}, links:[] },
        { id:"p10", title:"App Service avg response (ms)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_average_response_time{ResourceType="Microsoft.Web/sites",SubscriptionId=~"$subscription",ResourceName=~"$resource"}) * 1000',
          x:6,y:5,w:3,h:3, options:{unit:"ms"}, links:[] },
        { id:"p11", title:"App Service memory working set (by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_memory_working_set{ResourceType="Microsoft.Web/sites",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:9,y:5,w:3,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 4: SQL Database --
        { id:"p12", title:"SQL DB CPU % (by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_cpu_percent{ResourceType="Microsoft.Sql/servers/databases",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:8,w:4,h:3, options:{unit:"%"}, links:[] },
        { id:"p13", title:"SQL DB successful connections", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_connection_successful{ResourceType="Microsoft.Sql/servers/databases",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:4,y:8,w:4,h:3, options:{}, links:[] },
        { id:"p14", title:"SQL DB failed connections", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_connection_failed{ResourceType="Microsoft.Sql/servers/databases",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:8,y:8,w:4,h:3, options:{}, links:[] },

        // -- Row 5: Storage --
        { id:"p15", title:"Storage availability % (by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_availability{ResourceType="Microsoft.Storage/storageAccounts",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:11,w:6,h:3, options:{unit:"%"}, links:[] },
        { id:"p16", title:"Storage used capacity (bytes, by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_used_capacity{ResourceType="Microsoft.Storage/storageAccounts",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:6,y:11,w:6,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 6: AKS --
        { id:"p17", title:"AKS node CPU % (by cluster)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_node_cpu_usage_percentage{ResourceType="Microsoft.ContainerService/managedClusters",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:14,w:6,h:3, options:{unit:"%"}, links:[] },
        { id:"p18", title:"AKS node memory % (by cluster)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_node_memory_working_set_percentage{ResourceType="Microsoft.ContainerService/managedClusters",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:6,y:14,w:6,h:3, options:{unit:"%"}, links:[] },

        // -- Row 7: Cosmos DB --
        { id:"p19", title:"Cosmos DB total requests (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_total_requests{ResourceType="Microsoft.DocumentDB/databaseAccounts",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:17,w:6,h:3, options:{}, links:[] },
        { id:"p20", title:"Cosmos DB request units consumed (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_total_request_units{ResourceType="Microsoft.DocumentDB/databaseAccounts",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:6,y:17,w:6,h:3, options:{}, links:[] },

        // -- Row 8: Service Bus + Event Hub --
        { id:"p21", title:"Service Bus messages (in/out, by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_incoming_messages{ResourceType="Microsoft.ServiceBus/namespaces",SubscriptionId=~"$subscription",ResourceName=~"$resource"}) + sum by (ResourceName) (azure_monitor_outgoing_messages{ResourceType="Microsoft.ServiceBus/namespaces",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:20,w:4,h:3, options:{}, links:[] },
        { id:"p22", title:"Service Bus active connections", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_active_connections{ResourceType="Microsoft.ServiceBus/namespaces",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:4,y:20,w:4,h:3, options:{}, links:[] },
        { id:"p23", title:"Event Hub messages (in+out, by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_incoming_messages{ResourceType="Microsoft.EventHub/namespaces",SubscriptionId=~"$subscription",ResourceName=~"$resource"}) + sum by (ResourceName) (azure_monitor_outgoing_messages{ResourceType="Microsoft.EventHub/namespaces",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:8,y:20,w:4,h:3, options:{}, links:[] },

        // -- Row 9: Key Vault + Redis --
        { id:"p24", title:"Key Vault API hits (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_service_api_hit{ResourceType="Microsoft.KeyVault/vaults",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:0,y:23,w:3,h:3, options:{}, links:[] },
        { id:"p25", title:"Key Vault API latency (ms, avg)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_service_api_latency{ResourceType="Microsoft.KeyVault/vaults",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:3,y:23,w:3,h:3, options:{unit:"ms"}, links:[] },
        { id:"p26", title:"Redis memory % used (by resource)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ResourceName) (azure_monitor_usedmemorypercentage{ResourceType="Microsoft.Cache/Redis",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:6,y:23,w:3,h:3, options:{unit:"%"}, links:[] },
        { id:"p27", title:"Redis ops / sec (by resource)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ResourceName) (azure_monitor_operations_per_second{ResourceType="Microsoft.Cache/Redis",SubscriptionId=~"$subscription",ResourceName=~"$resource"})',
          x:9,y:23,w:3,h:3, options:{}, links:[] },
      ]}
  },
  {
    id: "gcp-overview", icon: "🟧", title: "GCP Overview", category: "Cloud",
    description: "Project-wide rollups for Compute Engine, Cloud Run, Cloud Functions, Cloud SQL, Pub/Sub, Cloud Storage, Cloud Load Balancing, and GKE — sourced from Google Cloud Monitoring via the pulseboard-cloud poller (60s timeSeries.list against the curated default metric plan). Filter the whole dashboard by $project. Requires: a connected GCP account in Portal → Cloud with the `gcp/monitoring` integration enabled (Workload Identity Federation, no service-account key stored). Series carry labels ResourceType + Aggregation=\"ALIGN_<aligner>\" + ProjectId + Workspace, plus resource and metric labels propagated from Cloud Monitoring. The `gcp_monitoring_*` names are PulseBoard's internal Prom-style series names; GCP metric types are snake-cased with `.googleapis.com` stripped.",
    metrics: [
      "gcp_monitoring_compute_instance_cpu_utilization",
      "gcp_monitoring_run_request_count",
      "gcp_monitoring_cloudsql_database_cpu_utilization",
      "gcp_monitoring_pubsub_subscription_num_undelivered_messages"
    ],
    alerts: { name:"GCP Alerts", intervalMs:60000, rules:[
      { name:"GCE VM CPU > 90%", lang:"promql",
        expr:'gcp_monitoring_compute_instance_cpu_utilization * 100',
        cmp:">", threshold:90, forMs:300000, severity:"warning",
        labels:{integration:"gcp-overview"},
        annotations:{summary:"GCE VM CPU > 90% in project {{ $labels.ProjectId }}"} },
      { name:"Cloud Run 5xx sustained", lang:"promql",
        expr:'sum by (ProjectId, service_name) (rate(gcp_monitoring_run_request_count{response_code_class="5xx"}[5m]))',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"gcp-overview"},
        annotations:{summary:"Cloud Run service {{ $labels.service_name }} returning 5xx in project {{ $labels.ProjectId }}"} },
      { name:"Cloud SQL CPU > 85%", lang:"promql",
        expr:'gcp_monitoring_cloudsql_database_cpu_utilization * 100',
        cmp:">", threshold:85, forMs:300000, severity:"critical",
        labels:{integration:"gcp-overview"},
        annotations:{summary:"Cloud SQL CPU > 85% in project {{ $labels.ProjectId }}"} },
      { name:"Pub/Sub backlog > 1000", lang:"promql",
        expr:'gcp_monitoring_pubsub_subscription_num_undelivered_messages',
        cmp:">", threshold:1000, forMs:300000, severity:"warning",
        labels:{integration:"gcp-overview"},
        annotations:{summary:"Pub/Sub subscription {{ $labels.subscription_id }} backlog > 1k in project {{ $labels.ProjectId }}"} },
      { name:"Load Balancer 5xx sustained", lang:"promql",
        expr:'sum by (ProjectId) (rate(gcp_monitoring_loadbalancing_https_request_count{response_code_class="500"}[5m]))',
        cmp:">", threshold:0, forMs:300000, severity:"warning",
        labels:{integration:"gcp-overview"},
        annotations:{summary:"Cloud Load Balancing returning 5xx in project {{ $labels.ProjectId }}"} },
    ]},
    dashboard: { title:"GCP Overview", timeRangeSec:3600, refreshSec:60,
      vars:[
        // Populated from labels stamped by the cloud-poller on every
        // Cloud Monitoring sample (cloud-poller/Poller.fs `extraLabels`).
        { name:"project", type:"query", label:"Project",
          query:"label_values(gcp_monitoring_compute_instance_cpu_utilization, ProjectId)",
          options:"", regex:"",
          multi:true, allOption:true, current:["$__all"], hide:false },
      ],
      panels:[
        // -- Row 1: top-line stats --
        { id:"p1", title:"GCP projects reporting", type:"stat", queryLang:"promql",
          expr:'count(count by (ProjectId) ({__name__=~"gcp_monitoring_.+",ProjectId=~"$project"}))',
          x:0,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p2", title:"Resource types reporting", type:"stat", queryLang:"promql",
          expr:'count(count by (ResourceType) ({__name__=~"gcp_monitoring_.+",ProjectId=~"$project"}))',
          x:3,y:0,w:3,h:2, options:{}, links:[] },
        { id:"p3", title:"Cloud Run requests / sec (sum)", type:"stat", queryLang:"promql",
          expr:'sum(rate(gcp_monitoring_run_request_count{ProjectId=~"$project"}[5m]))',
          x:6,y:0,w:3,h:2, options:{unit:"ops"}, links:[] },
        { id:"p4", title:"Pub/Sub backlog (sum)", type:"stat", queryLang:"promql",
          expr:'sum(gcp_monitoring_pubsub_subscription_num_undelivered_messages{ProjectId=~"$project"})',
          x:9,y:0,w:3,h:2, options:{}, links:[] },

        // -- Row 2: Compute Engine --
        { id:"p5", title:"GCE VM CPU % (avg, by project)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ProjectId) (gcp_monitoring_compute_instance_cpu_utilization{ProjectId=~"$project"}) * 100',
          x:0,y:2,w:6,h:3, options:{unit:"%"}, links:[] },
        { id:"p6", title:"GCE network received (bytes/poll, by project)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId) (gcp_monitoring_compute_instance_network_received_bytes_count{ProjectId=~"$project"})',
          x:6,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },
        { id:"p7", title:"GCE network sent (bytes/poll, by project)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId) (gcp_monitoring_compute_instance_network_sent_bytes_count{ProjectId=~"$project"})',
          x:9,y:2,w:3,h:3, options:{unit:"bytes"}, links:[] },

        // -- Row 3: Cloud Run --
        { id:"p8", title:"Cloud Run requests (by service)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId, service_name) (gcp_monitoring_run_request_count{ProjectId=~"$project"})',
          x:0,y:5,w:4,h:3, options:{}, links:[] },
        { id:"p9", title:"Cloud Run 5xx (by service)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId, service_name) (gcp_monitoring_run_request_count{ProjectId=~"$project",response_code_class="5xx"})',
          x:4,y:5,w:4,h:3, options:{}, links:[] },
        { id:"p10", title:"Cloud Run container CPU utilization (avg, by service)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ProjectId, service_name) (gcp_monitoring_run_container_cpu_utilizations{ProjectId=~"$project"})',
          x:8,y:5,w:4,h:3, options:{unit:"%"}, links:[] },

        // -- Row 4: Cloud Functions + Cloud SQL --
        { id:"p11", title:"Cloud Functions executions (by function)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId, function_name) (gcp_monitoring_cloudfunctions_function_execution_count{ProjectId=~"$project"})',
          x:0,y:8,w:6,h:3, options:{}, links:[] },
        { id:"p12", title:"Cloud SQL CPU % (avg, by database)", type:"timeseries", queryLang:"promql",
          expr:'avg by (ProjectId, database_id) (gcp_monitoring_cloudsql_database_cpu_utilization{ProjectId=~"$project"}) * 100',
          x:6,y:8,w:6,h:3, options:{unit:"%"}, links:[] },

        // -- Row 5: Pub/Sub --
        { id:"p13", title:"Pub/Sub undelivered messages (by subscription)", type:"timeseries", queryLang:"promql",
          expr:'gcp_monitoring_pubsub_subscription_num_undelivered_messages{ProjectId=~"$project"}',
          x:0,y:11,w:12,h:3, options:{}, links:[] },

        // -- Row 6: Cloud Storage + Load Balancing --
        { id:"p14", title:"Cloud Storage api requests (by project)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId) (gcp_monitoring_storage_api_request_count{ProjectId=~"$project"})',
          x:0,y:14,w:6,h:3, options:{}, links:[] },
        { id:"p15", title:"Load Balancing HTTPS requests (by project)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId) (gcp_monitoring_loadbalancing_https_request_count{ProjectId=~"$project"})',
          x:6,y:14,w:6,h:3, options:{}, links:[] },

        // -- Row 7: GKE container --
        { id:"p16", title:"GKE container CPU core-usage time (by project)", type:"timeseries", queryLang:"promql",
          expr:'sum by (ProjectId) (rate(gcp_monitoring_kubernetes_io_container_cpu_core_usage_time{ProjectId=~"$project"}[5m]))',
          x:0,y:17,w:12,h:3, options:{}, links:[] },
      ]}
  },
];
