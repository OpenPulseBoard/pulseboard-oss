...
| [`site.toml`](site.toml) | `pulseboard-site` | flycast-only | `pulseboard --site-only`. Marketing + signup proxy. |
| [`provisioner.toml`](provisioner.toml) | `pulseboard-provisioner` | flycast-only | `pulseboard --mode=provisioner`. Holds Fly API token + Postgres. |
| [`workspace.toml`](workspace.toml) | `pb-<slug>` (one per customer) | flycast-only | `pulseboard --multi-tenant` template the provisioner clones via the Fly Machines API. |
...
```bash
curl -fsSL https://pulseboard.cloud/ | head -20             # site-only home
